...
namespace multi_bitset {
	...
	public:
		...
		bool is_equal(const Mat& mat) const {
			for (int y = 0; y < H; y++) {
				if (this->bits_[y] != mat.bits_[y]) return false;
			}
			return true;
		}
		...
		bool is_any_equal(const Mat& mat) const {
			for (int y = 0; y < H; y++) {
				if ((this->bits_[y] & mat.bits_[y]).any()) return true;
			}
			return false;
		}
...
private:
	...
	int getDistanceToNearestPoint() {
		...
		while (!que.empty()) {
			...
			que.pop_front();
			if (this->points_[tmp_cod.y_][tmp_cod.x_] > 0) return tmp_cod.distance_;
	...
	void init_hash() {
		...
		for (int y = 0; y < H; y++) {
			for (int x = 0; x < W; x++) {
				...
				if (point > 0) hash_ ^= zobrist_hash::points[y][x][point];
			}
		}
	}
...
public:
	...
	ScoreType evaluated_score_ = 0;	// 探索木で評価したスコア
	int first_action_ = -1;			// 探索木のルートノードで最初に選択した行動
	uint64_t hash_ = 0;				// 同一盤面判定に使うハッシュ

	// MazeStateByBitSetから参照できるようにpublicにする
	...
	WallMazeState(const int seed) {
		...
		for (int y = 1; y < H; y += 2) {
			for (int x = 1; x < W; x += 2) {
				...
				// ここで(ty, tx)は1マス置きの位置
				if (ty == character_.y_ && tx == character_.x_) continue;

				this->walls_[ty][tx] = 1;
				int direction_size = 3; // (右、左、下の隣接マスを壁方向にする。)
				if (y == 1) direction_size = 4; // 最初だけ上方向の隣接マスも壁候補にする。
...
private:
	static constexpr const int dx[4] = { 1, -1, 0, 0 };	// 右、左、下、上の移動方向のx成分
	static constexpr const int dy[4] = { 0, 0, 1, -1 };	// 右、左、下、上の移動方向のy成分
	...
	int getDistanceToNearestPoint() {
		...
		for (int depth = 0;; ++depth) {
			if (now.is_any_equal(this->whole_point_mat_)) return depth;
			auto next = now;
			next.expand();
			next.andeq_not(this->walls_);
			if (next.is_equal(now)) break;
			now = next;
		}
		return H * W;
	}
...
public:
	...
	MazeStateByBitSet(const WallMazeState& state) : turn_(state.turn_), character_(state.character_.y_, state.character_.x_), game_score_(state.game_score_) {
		for (int y = 0; y < H; y++) {
			for (int x = 0; x < W; x++) {
				if (state.walls_[y][x]) this->walls_.set(y, x);
	...
	for (int t = 0; t < beam_depth; t++) {
		...
		for (int i = 0; i < beam_width; i++) {
			...
			for (const auto& action : legal_actions) {
				...
				if (t >= 1 && hash_check.count(next_state.hash_) > 0) continue;
...
void testAiScore(const StringAIPair& ai, const int game_number) {
	...
	for (int i = 0; i < game_number; i++) {
		...
		while (!state.isDone()) state.advance(ai.second(MazeStateByBitSet(state)));
...
// game_number種類のゲーム盤面について
// per_game_number回の処理を行うのに必要な平均時間(ms)を計測して表示する。
