...
private:
	...
	bool isWinner(const uint64_t board) {
		...
		if ((tmp_board & (tmp_board >> 14)) != 0) return true;

		// \方向の連結判定
		tmp_board = board & (board >> 6);
		if ((tmp_board & (tmp_board >> 12)) != 0) return true;

		// "／"方向の連結判定
		tmp_board = board & (board >> 8);
		if ((tmp_board & (tmp_board >> 16)) != 0) return true;

		// 縦方向の連結判定
		tmp_board = board & (board >> 1);
		if ((tmp_board & (tmp_board >> 2)) != 0) return true;
...
public:
	...
	ConnectFourStateByBitSet(const ConnectFourState& state) : is_first_(state.is_first_) {
		...
		for (int y = 0; y < H; y++) {
			for (int x = 0; x < W; x++) {
				int index = x * (H + 1) + y;
				if (state.my_board_[y][x] == 1) this->my_board_ |= 1ULL << index;
				if (state.my_board_[y][x] == 1 || state.enemy_board_[y][x] == 1) this->all_board_ |= 1ULL << index;
			}
		}
	}
	bool isDone() const {
	...
	void advance(const int action) {
		...
		if (isWinner(this->my_board_ ^ this->all_board_)) this->winning_status_ = WinningStatus::LOSE;
		else if (this->all_board_ == filled) this->winning_status_ = WinningStatus::DRAW;
	}
	...
	vector<int> legalActions() const {
		...
		for (int x = 0; x < W; x++) {
			if ((filter & possible) != 0) actions.emplace_back(x);
			filter <<= 7;
		}
		return actions;
	}
