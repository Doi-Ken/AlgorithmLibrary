...
public:
	AlternateMazeState(const SimultaneousMazeState& base_state, const int player_id)
		: points_(base_state.points_),
		turn_(base_state.turn_ * 2), // 同時着手の1ターンは交互着手では2ターン分
		characters_(
			player_id == 0 ?
			base_state.characters_ :
			vector<Character>{ base_state.characters_[1], base_state.characters_[0] }
		) {}
	...
	WinningStatus getWinningStatus() const {
		...
		else return WinningStatus::NONE;
	}
...
namespace alternate_montecarlo {
...
using alternate_montecarlo::mctsAction;
...
namespace montecarlo {
	...
	public:
		...
		Node& nextChildNode() {
			...
			double t = 0;
			for (auto& child_nodes : this->child_nodeses_) {
				for (auto& child_node : child_nodes) t += child_node.n_;
			}

			int best_is[] = { -1, 1 };

			// プレイヤー0側の行動選択
			// 思考するプレイヤーがどちらかに関係なくここではプレイヤー0側の行動
			double best_value = -INF;
			for (int i = 0; i < this->child_nodeses_.size(); i++) {
				const auto& child_nodes = this->child_nodeses_[i];
				double w = 0;
				double n = 0;
				...
				double ucb1_value = w / n + (double)C * sqrt(2. * log(t) / n);
				if (ucb1_value > best_value) {
					best_is[0] = i;
					best_value = ucb1_value;
				}
			...
			// プレイヤー1側の行動選択
			// 思考するプレイヤーがどちらかに関係なくここではプレイヤー1側の行動
			best_value = -INF;
			for (int j = 0; j < this->child_nodeses_[0].size(); j++) {
				...
				double ucb1_value = 1. - w / n + (double)C * sqrt(2. * log(t) / n);
				if (ucb1_value > best_value) {
					best_is[1] = j;
					best_value = ucb1_value;
				}
