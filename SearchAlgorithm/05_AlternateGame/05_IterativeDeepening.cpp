...
void playGame(const int seed) {
	...
	while (!state.isDone()) {
		// 1p
		{
			...
			cout << state.toString() << endl;
			if (state.isDone()) {
				// (a-2) a-1で2P視点になっているので、WINなら2pの勝利
		...
		// 2p
		{
			...
			cout << state.toString() << endl;
			if (state.isDone()) {
				// (b-2) b-1で1P視点になっているので、WINなら1pの勝利
				switch (state.getWinningStatus()) {
				case(WinningStatus::WIN):
					cout << "winner: " << "1p" << endl;
					break;
...
// ゲームをgame_numberx2(先手後手を交代)回プレイしてaisの0番目のAIの勝率を表示する。
void testFirstPlayerWinRate(
...
vector<State> getSampleStates(const int game_number) {
...
void calculateExecutionSpeed(const StringAIPair& ai, const vector<State>& states) {
...
namespace alphabeta {
	...
	ScoreType alphaBetaScore(
		...
		if (state.isDone() || depth == 0) return state.getScore();
		auto legal_actions = state.legalActions();
...
namespace iterativedeepening {
	...
	int alphaBetaActionWithTimeThreshold(
		const State& state, 
		const int depth,
		const TimeKeeper &time_keeper
	) {
		...
		for (const auto action : state.legalActions()) {
			State next_state = state;
			next_state.advance(action);
			ScoreType score = -alphaBetaScore(next_state, -beta, -alpha, depth, time_keeper);
			if (time_keeper.isTimeOver()) return 0;
			if (score > alpha) {
				best_action = action;
				alpha = score;
