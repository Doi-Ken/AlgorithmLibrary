...
namespace montecarlo {
	...
	// 制限時間を指定してMCTSで行動を決定する
	int mctsActionWithTimeThreshold(const State& state, const int64_t time_threshold, const bool is_print) {
...
namespace thunder {
	...
	int thunderSearchActionWithTimeThreshold(const State& state, const int64_t time_threshold, const bool is_print) {
...
int main() {
	...
	auto ais = array<StringAIPair, 2>{
		StringAIPair("thunderSearchActionWithTimeThreshold 1ms", [](const State& state)
			{ return thunder::thunderSearchActionWithTimeThreshold(state, 1, false); }),
			StringAIPair("mctsActionWithTimeThreshold 1ms", [](const State& state)
				{return montecarlo::mctsActionWithTimeThreshold(state, 1, false); })
	};
